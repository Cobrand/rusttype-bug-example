...
fn main() {
    let font = if let Some(font_path) = std::env::args().nth(1) {
        ...
        eprintln!("No font specified ... using Bitter-Regular.ttf");
        let font_data = include_bytes!("../Bitter-Regular.otf");
    ...
    let glyphs: Vec<_> = font.layout("Gauntl", scale, offset).collect();
